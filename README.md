...
jobs:
  release-pr:
    uses: powerman/workflows/.github/workflows/release-pr.yml@v0.4.2
    # with:
...
1. **Manual version change + new commits**:
   User changes version in PR title while new commits are being pushed.
2. **Delayed workflow execution**:
   Release PR is merged before the latest changes are reflected.
